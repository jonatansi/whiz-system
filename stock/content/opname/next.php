<?php
$d=mysqli_fetch_array(mysqli_query($conn,"SELECT status_id FROM opname WHERE id='$_POST[id]'"));
if($d['status_id']=='350'){
    $next_status_id = 360;
    $title = "On Progress";
}
else if($d['status_id']=='360'){
    $next_status_id = 365;
    $title = "Completed";
}
?>
<form action="opname-next-action" method="POST" enctype="multipart/form-data" id="form_crud">
